...
@Manifest(name = "Lodestone Activator", authors = { "Tutorial Con" }, version = 1.0D, description = "Will activate all the Lodestones.", website = "http://www.TutorialCon.com/")
public class LodestoneActivator extends ActiveScript implements Task,
		PaintListener {
	...
	protected void setup() {
		...
		final Strategy a_checkLodestones = new Strategy(t_checkLodestones,
				t_checkLodestones);
	...
	public void updateSettings() {
		...
		b_done = new boolean[] { false, false, false, false, false, false,
				false, false, false, false, false };
		w_done = new WidgetChild[] { LODE_DRAYNOR, LODE_FALADOR,
				LODE_PORTSARIM, LODE_TAVERLY, LODE_CATHERBY, LODE_SEERS,
				LODE_ARDOUGNE, LODE_YANILLE, LODE_ALKHARID, LODE_VARROCK,
				LODE_EDGEVILLE };
		t_done = new int[] { REQ_LODE_DRAYNOR, REQ_LODE_FALADOR,
				REQ_LODE_PORTSARIM, REQ_LODE_TAVERLY, REQ_LODE_CATHERBY,
				REQ_LODE_SEERS, REQ_LODE_ARDOUGNE, REQ_LODE_YANILLE,
				REQ_LODE_ALKHARID, REQ_LODE_VARROCK, REQ_LODE_EDGEVILLE };
	}
	...
	public void run() {
		...
		if (!walking) {
			...
			if (!(playerLocation.getX() <= curPath[curProgress].getX() + 1
					&& playerLocation.getX() >= curPath[curProgress].getX() - 1
					&& playerLocation.getY() <= curPath[curProgress].getY() + 1 && playerLocation
					.getY() >= curPath[curProgress].getY() - 1)) {
				homePort(wcs[0]);
			}
			...
			if (!resting) {
				if (Integer.parseInt(BTN_RUN.getText()) > 50
						&& Settings.get(SETTING_RUN) != 1) {
						...
						SceneObject o = SceneEntities
								.getNearest(new Filter<SceneObject>() {
									...
									public boolean accept(final SceneObject o) {
										return o != null
												&& o.getId() == getLodestoneID(targetLodestone);
									}
								});
						o.interact("Activate");
				...
				Tile playerLocation = Players.getLocal().getLocation();
				if (playerLocation.getX() <= curPath[curProgress].getX() + 3
						&& playerLocation.getX() >= curPath[curProgress].getX() - 3
						&& playerLocation.getY() <= curPath[curProgress].getY() + 3
						&& playerLocation.getY() >= curPath[curProgress].getY() - 3) {
					traversing = false;
					curProgress++;
				}
			...
			} else if (Integer.parseInt(BTN_RUN.getText()) >= 100
					|| Players.getLocal().isInCombat()) {
				resting = false;
			}
		}
	...
	public Tile[] getWalkPath(WidgetChild wc) {
		if (wc == LODE_DRAYNOR) {
			return new Tile[] { new Tile(3233, 3221, 0),
					new Tile(3225, 3236, 0), new Tile(3219, 3245, 0),
					new Tile(3212, 3246, 0), new Tile(3196, 3248, 0),
					new Tile(3183, 3251, 0), new Tile(3177, 3258, 0),
					new Tile(3166, 3263, 0), new Tile(3154, 3260, 0),
					new Tile(3149, 3264, 0), new Tile(3135, 3272, 0),
					new Tile(3134, 3289, 0), new Tile(3118, 3295, 0),
					new Tile(3105, 3298, 0) };
		} else if (wc == LODE_PORTSARIM) {
			return new Tile[] { new Tile(3105, 3298, 0),
					new Tile(3090, 3289, 0), new Tile(3076, 3280, 0),
					new Tile(3063, 3269, 0), new Tile(3058, 3256, 0),
					new Tile(3045, 3246, 0), new Tile(3031, 3236, 0),
					new Tile(3028, 3221, 0), new Tile(3012, 3216, 0) };
		} else if (wc == LODE_FALADOR) {
			return new Tile[] { new Tile(3012, 3216, 0),
					new Tile(3008, 3232, 0), new Tile(3007, 3246, 0),
					new Tile(3008, 3259, 0), new Tile(3008, 3274, 0),
					new Tile(3005, 3285, 0), new Tile(3004, 3296, 0),
					new Tile(3006, 3312, 0), new Tile(3005, 3328, 0),
					new Tile(3006, 3344, 0), new Tile(3001, 3357, 0),
					new Tile(2993, 3368, 0), new Tile(2981, 3376, 0),
					new Tile(2966, 3381, 0), new Tile(2966, 3397, 0),
					new Tile(2967, 3403, 0) };
		} else if (wc == LODE_EDGEVILLE) {
			return new Tile[] { new Tile(2967, 3403, 0),
					new Tile(2978, 3415, 0), new Tile(2988, 3427, 0),
					new Tile(3004, 3432, 0), new Tile(3020, 3435, 0),
					new Tile(3032, 3446, 0), new Tile(3038, 3460, 0),
					new Tile(3051, 3470, 0), new Tile(3063, 3482, 0),
					new Tile(3064, 3496, 0), new Tile(3067, 3505, 0) };
		} else if (wc == LODE_VARROCK) {
			return new Tile[] { new Tile(3067, 3505, 0),
					new Tile(3082, 3504, 0), new Tile(3087, 3489, 0),
					new Tile(3080, 3474, 0), new Tile(3088, 3461, 0),
					new Tile(3091, 3446, 0), new Tile(3094, 3430, 0),
					new Tile(3106, 3421, 0), new Tile(3121, 3416, 0),
					new Tile(3133, 3408, 0), new Tile(3145, 3400, 0),
					new Tile(3158, 3395, 0), new Tile(3173, 3391, 0),
					new Tile(3182, 3380, 0), new Tile(3198, 3380, 0),
					new Tile(3208, 3376, 0), new Tile(3214, 3376, 0) };
		} else if (wc == LODE_ALKHARID) {
			return new Tile[] { new Tile(3214, 3376, 0),
					new Tile(3220, 3361, 0), new Tile(3225, 3346, 0),
					new Tile(3239, 3337, 0), new Tile(3255, 3334, 0),
					new Tile(3270, 3331, 0), new Tile(3284, 3325, 0),
					new Tile(3284, 3310, 0), new Tile(3284, 3297, 0),
					new Tile(3285, 3282, 0), new Tile(3286, 3267, 0),
					new Tile(3287, 3252, 0), new Tile(3289, 3237, 0),
					new Tile(3292, 3221, 0), new Tile(3298, 3207, 0),
					new Tile(3295, 3192, 0), new Tile(3297, 3184, 0) };
		} else {
			return null;
		}
	}
	...
	public boolean walkTileMM(Tile tile, int rnd) {
		...
		int x = 627, y = 85;
		int dx = (int) (4 * distance * Math.cos(angle)) + 4
				* Random.nextGaussian(0, rnd, 1);
		int dy = (int) (4 * distance * Math.sin(angle)) + 4
				* Random.nextGaussian(0, rnd, 1);
		return Mouse.click(x + dx, y - dy, true);
	}

	public int distanceTo(Tile tile) {
		return (int) Calculations.distance(Players.getLocal().getLocation(),
				tile);
	}
	...
	private class Click {
		...
		public void drawTo(Graphics2D g) {
			...
			g.drawLine((int) (x + 5 * Math.cos(radians)),
					(int) (y + 5 * Math.sin(radians)),
	...
	public void onRepaint(Graphics gra) {
		...
		g.drawLine(
				(int) Math.round(clientCursor.x + 7 * Math.cos(radians)),
				(int) Math.round(clientCursor.y + 7 * Math.sin(radians)),
				(int) Math.round(clientCursor.x + 7
						* Math.cos(radians + Math.PI)),
				(int) Math.round(clientCursor.y + 7
						* Math.sin(radians + Math.PI)));
		g.drawLine(
				(int) Math.round(clientCursor.x + 7
						* Math.cos(radians + HALF_PI)),
				(int) Math.round(clientCursor.y + 7
						* Math.sin(radians + HALF_PI)),
				(int) Math.round(clientCursor.x + 7
						* Math.cos(radians + THREE_HALF_PI)),
				(int) Math.round(clientCursor.y + 7
	...
	private class SetupBot extends Strategy implements Task {
		...
		public void run() {
			...
			// Just incase, for some reason, the previous method failed
			if (Tabs.getCurrent() == Tabs.MAGIC) {
				if (SORT_TELE.getTextureId() != REQ_ON_TEX && clicked < 1) {
					SORT_TELE.interact("Sort");
					clicked = 1;
				} else if (FILT_CMB.getTextureId() == REQ_ON_TEX && clicked < 2) {
